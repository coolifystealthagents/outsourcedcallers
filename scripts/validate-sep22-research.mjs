...
const manifest = JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-22/research-outaaaaa-62.json', 'utf8'));
assert.equal(manifest.publicationDate, '2026-09-22');
...
const priorSource = fs.readFileSync('app/sep18-run2-research.ts', 'utf8');
...
for (const entry of manifest.entries) {
  ...
  assert.ok(html.includes('September 22, 2026'), `${entry.route} visible date`);
  assert.ok(html.includes('"datePublished":"2026-09-22"'), `${entry.route} structured date`);
  ...
  assert.ok(html.includes('checked 2026-09-22'), `${entry.route} source checked dates`);
