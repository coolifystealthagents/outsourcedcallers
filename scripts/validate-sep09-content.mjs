...
const blog=JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-09/blog.json','utf8'));
const research=JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-09/research.json','utf8'));
...
for(const {route} of entries){const html=await get(route),canonical=`${canonicalBase}${route}`;
 ...
 assert.ok(html.includes('September 9, 2026'),`${route} visible date`);
 assert.ok(html.includes('"datePublished":"2026-09-09"'),`${route} structured date`);
