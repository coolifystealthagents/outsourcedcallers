...
const manifest = JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-23/research-outaaaaa-64.json', 'utf8'));
assert.equal(manifest.publicationDate, '2026-09-23');
...
const priorFiles = fs.readdirSync('app').filter((name) => name.endsWith('.ts') && name !== 'sep23-research.ts');
const priorSource = priorFiles.map((name) => fs.readFileSync(`app/${name}`, 'utf8')).join('\n');
for (const entry of manifest.entries) assert.ok(!priorSource.includes(`slug: '${entry.slug}'`), `${entry.slug} must be new`);
...
for (const entry of manifest.entries) {
  ...
  assert.ok(html.includes('September 23, 2026'), `${entry.route} visible date`);
  assert.ok(html.includes('"datePublished":"2026-09-23"'), `${entry.route} structured date`);
  ...
  assert.ok(title && !titles.has(title), `${entry.route} unique title`); titles.add(title);
  const article = html.match(/<article[\s\S]*?<\/article>/)?.[0];
  assert.ok(article, `${entry.route} article`);
  const words = strip(article).split(/\s+/).filter(Boolean).length;
  assert.ok(words >= 1200, `${entry.route} ${words} words`);
  const contentHash = crypto.createHash('sha256').update(article).digest('hex');
  assert.ok(!hashes.has(contentHash), `${entry.route} unique hash`); hashes.add(contentHash);
  assert.ok(html.includes('Sources:'), `${entry.route} sources`);
  assert.ok(html.includes('checked 2026-09-23'), `${entry.route} source checked dates`);
  assert.ok(html.includes('Methodology.'), `${entry.route} methodology`);
  assert.ok(html.includes('Limitations and non-claims.'), `${entry.route} limitations`);
