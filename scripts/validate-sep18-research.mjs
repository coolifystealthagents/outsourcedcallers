...
assert.equal(manifest.campaignDate, '2026-09-18');
...
for (const entry of manifest.entries) {
  ...
  assert.ok(html.includes('September 18, 2026'), `${entry.route} visible date`);
  assert.ok(html.includes('"datePublished":"2026-09-18"'), `${entry.route} structured date`);
  assert.ok(!html.includes('"dateModified"') || html.includes('"dateModified":"2026-09-18"'), `${entry.route} truthful modified date`);
  assert.ok(index.includes(`href="${entry.route}"`), `${entry.route} research index`);
  ...
  assert.ok(title && !titles.has(title), `${entry.route} unique title`);
  titles.add(title);
  ...
  assert.ok(words >= 1200, `${entry.route} ${words} words; expected >=1200`);
  const hash = crypto.createHash('sha256').update(article).digest('hex');
  assert.ok(!hashes.has(hash), `${entry.route} unique article hash`);
  hashes.add(hash);
  assert.ok(html.includes('Sources:'), `${entry.route} source section`);
  assert.ok(html.includes('checked 2026-09-18'), `${entry.route} checked dates`);
  ...
  results.push({ route: entry.route, words, contentHash: hash });
}
