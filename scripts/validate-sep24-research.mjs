...
const manifest = JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-24/research-outaaaaa-66.json', 'utf8'));
assert.equal(manifest.publicationDate, '2026-09-24');
...
const priorFiles = fs.readdirSync('app').filter((name) => name.endsWith('.ts') && name !== 'sep24-research.ts');
...
for (const entry of manifest.entries) {
  ...
  assert.ok(html.includes('September 24, 2026'), `${entry.route} visible date`);
  assert.ok(html.includes('"datePublished":"2026-09-24"'), `${entry.route} structured date`);
  ...
  assert.ok(html.includes('Sources:') && html.includes('checked 2026-09-24'), `${entry.route} sourced and checked`);
