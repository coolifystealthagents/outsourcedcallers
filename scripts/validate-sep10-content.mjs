...
const blog=JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-10/blog.json','utf8'));
const research=JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-10/research.json','utf8'));
...
for(const {route} of entries){const html=await get(route),canonical=`${canonicalBase}${route}`;
 ...
 assert.ok(html.includes('September 10, 2026'),`${route} visible date`);
 assert.ok(html.includes('"datePublished":"2026-09-10"'),`${route} structured date`);
